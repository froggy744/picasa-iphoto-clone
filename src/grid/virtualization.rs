...
impl Gallery {
    ...
    /// Animate a window-resize column transition with the same scale+slide
    /// presentation used by manual zoom, without changing the saved zoom size.
    ///
    /// Phase 1 keeps the old outer layout width and temporarily scales tiles
    /// just far enough for GridView to cross into the target column count.
    /// Phase 2 switches to the latest physical viewport width (where that
    ...
    pub fn update_width_animated(self: &Rc<Self>, width: i32) {
        ...
        self.resize_animation_pending_width.set(width);

        // Manual zoom owns the shared layout-width freeze while active. The
        // normal resize tick will call us again after it finishes.
        if self.zoom_animation_layout_width.get().is_some()
            && !self.resize_animation_active.get()
        {
            return;
        }

        if self.resize_animation_active.get() {
            return;
        }

        let old_columns = self.current_columns.get();
        let target_columns = self.columns_for_width(width);
        if target_columns == old_columns {
            self.update_width(width);
            return;
        }

        // Keep the legacy Folder ListView on its immediate path; its model rows
        // still encode column membership.
        ...
        let canonical_width = self.tile_width.get().max(1);
        let canonical_height = self.tile_height.get().max(1);
        let old_layout_width = if self.last_layout_width.get() > 100 {
        ...
        let generation = self.resize_animation_generation.get().wrapping_add(1);
        self.resize_animation_generation.set(generation);
        self.resize_animation_active.set(true);
        self.zoom_animation_layout_width.set(Some(old_layout_width));
        set_grid_zoom_animation_active(true);

        let started = Instant::now();
        let this = self.clone();
        self.root.add_tick_callback(move |_, _| {
            if this.resize_animation_generation.get() != generation {
                return glib::ControlFlow::Break;
            }

            let linear = (started.elapsed().as_secs_f64() * 1000.0 / RESIZE_REFLOW_MS)
                .clamp(0.0, 1.0);

            let (from_width, to_width, from_height, to_height, phase_t) =
                if linear < 0.5 {
                    (
                        canonical_width,
                        bridge_width,
                    ...
                    // By halfway the GridView has already crossed into the new
                    // column count. Move the layout freeze to the latest real
                    // window width, then return tiles to their canonical size.
                    let latest_width = this.resize_animation_pending_width.get().max(101);
                    this.zoom_animation_layout_width.set(Some(latest_width));
                    (
                        bridge_width,
                        canonical_width,
                        bridge_height,
            ...
            let eased = 1.0 - (1.0 - phase_t).powi(3);
            let frame_width = (from_width as f64
                + (to_width - from_width) as f64 * eased)
                .round() as i32;
            let frame_height = (from_height as f64
                + (to_height - from_height) as f64 * eased)
                .round() as i32;

            this.apply_tile_geometry(frame_width, frame_height, false);

            if linear >= 1.0 {
                this.apply_tile_geometry(canonical_width, canonical_height, false);
                this.zoom_animation_layout_width.set(None);
                this.resize_animation_active.set(false);
                set_grid_zoom_animation_active(false);

                // If the user dragged through another column boundary while
                // this transition was running, immediately begin the next
                // transition from the latest physical width.
                let latest_width = this.resize_animation_pending_width.get();
                this.update_width_animated(latest_width);
                glib::ControlFlow::Break
            } else {
