...
## Picasa-style sticky Folder heading

Status: **IMPLEMENTED — visual validation pending**

Gallery v2 now promotes the existing lightweight sticky group header into the Folder-mode section indicator instead of reintroducing in-flow Folder row models.

Commits:

- `1d62ac5` — rebuild the sticky group header as a compact icon/title/count line with an optional separator
- `9cac974` — enable Folder-specific styling only while Folder grouping is active
- `84cc406` — clear Folder-only adornments when no active range exists
- `6713ff6` — update the window layout documentation for the shared sticky heading surface

Folder-mode presentation now uses the existing theme classes already defined for the former section headers:
...
- direct Gallery v2 Folder mode still binds the flat photo model directly to `GtkGridView`
- no header objects are inserted into the photo model
- no nested GridViews/ListViews are introduced
- the sticky heading follows the current `GroupRange` as scrolling changes the leading photo index
- Folder mode shows a folder icon, folder name, photo count and themed separator
- Day/Month/History grouping keeps the previous generic heading style without Folder adornments
